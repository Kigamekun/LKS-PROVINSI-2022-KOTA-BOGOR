...
class AIOWPSecurity_Utility_Htaccess {
	...
	public static function write_to_htaccess() {
		...
		if (!function_exists('get_home_path')) require_once(ABSPATH. '/admin/includes/file.php');
	...
	public static function getrules_enable_brute_force_prevention() {
		...
		if ($aio_wp_security->configs->get_value('aiowps_enable_brute_force_attack_prevention') == '1') {
			...
			$rules .= 'RewriteCond %{REQUEST_URI} (admin|wp-login)' . PHP_EOL;// If URI contains admin or wp-login
			if ($aio_wp_security->configs->get_value('aiowps_brute_force_attack_prevention_ajax_exception') == '1') {
				$rules .= 'RewriteCond %{REQUEST_URI} !(admin/admin-ajax.php)' . PHP_EOL; // To allow ajax requests through
			}
	...
	/**
	 * This function will write some directives to allow IPs in the whitelist to access wp-login.php or admin
