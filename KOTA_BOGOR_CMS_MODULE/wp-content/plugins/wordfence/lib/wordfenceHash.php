...
class wordfenceHash {
	...
	public function __construct($scannedFiles, $themes, $plugins, $engine, $malwarePrefixesHash, $coreHashesHash, $scanMode) {
		...
		if($this->coreUnknownEnabled){ $this->status['coreUnknown'] = wfIssues::statusStart(__("Scanning for unknown files in admin and wp-includes", 'wordfence')); $this->engine->scanController()->startStage(wfScanner::STAGE_FILE_CHANGES); } else { wfIssues::statusDisabled(__("Skipping unknown core file scan", 'wordfence')); }
		...
		if ($wfHash) {
			...
			if ($allowKnownFileScan) {
				if (isset($this->knownFiles['core'][$wordpressPath])) {
					...
					$restrictedWordPressFolders = array(ABSPATH . 'admin/', ABSPATH . WPINC . '/');
					$added = false;
