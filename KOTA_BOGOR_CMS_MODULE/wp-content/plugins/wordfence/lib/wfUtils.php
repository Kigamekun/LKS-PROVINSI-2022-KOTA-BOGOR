...
class wfUtils {
	...
	public static function isAdminPageMU(){
		if(preg_match('/^[\/a-zA-Z0-9\-\_\s\+\~\!\^\.]*\/admin\/network\//', $_SERVER['REQUEST_URI'])){
			return true;
		}
		return false;
	}
	...
	public static function wpAdminURL($path = '', $scheme = null) {
		if (!is_multisite()) {
			$adminURL = self::wpSiteURL('admin/', $scheme);
		}
		else {
			$adminURL = self::wpSiteURL('admin/network/', $scheme);
		}
	...
	public static function isFullSSL() {
		return is_ssl() && parse_url(self::wpHomeURL(), PHP_URL_SCHEME) === 'https'; //It's possible for only admin to be SSL so we check the home URL too
	}
