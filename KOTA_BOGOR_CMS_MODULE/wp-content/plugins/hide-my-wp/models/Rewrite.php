...
class HMWP_Models_Rewrite
{
    ...
    public function setRewriteRules()
    {
        ...
        include_once ABSPATH . 'admin/includes/file.php';
    ...
    public function network_admin_url( $url, $path = '', $scheme = null )
    {
        ...
        if (HMWP_Classes_Tools::getOption('hmwp_admin_url') == 'admin' ) {
            return $url;
        }
        ...
        if (HMWP_Classes_Tools::getValue('noredirect') || HMWP_Classes_Tools::getOption('hmwp_remove_third_hooks') ) {
            ...
            if (isset($user) && isset($user->ID) ) {
                if (!is_wp_error($user) && empty($_REQUEST['reauth']) ) {
                    ...
                    //If admin redirect
                    if ((empty($redirect) || $redirect == 'admin/' || $redirect == admin_url()) ) {
    ...
    public function hideUrls()
    {
        ...
        if(HMWP_Classes_Tools::doHideURLs() ) {
            ...
            if (is_user_logged_in()) {
                ...
                //Hide the admin for logged users
                ...
                if ($url <> '') {
                    ...
                    if (HMWP_Classes_Tools::$default['hmwp_admin_url'] <> HMWP_Classes_Tools::getOption('hmwp_admin_url')) {
                        ...
                        if (strpos($url, '/admin') !== false && strpos($url, admin_url('admin-ajax.php', 'relative')) === false && HMWP_Classes_Tools::getOption('hmwp_hide_admin')) {
                            $this->getNotFound($url);
                        }
                    }
        ...
        if (HMWP_Classes_Tools::isContentHeader(array('text/plain'))) {
            ...
            if($content <> '') {
                ...
                foreach ($array as $index => $row) {
                    if (strpos($row, '/admin') !== false || strpos($row, '/wp-login.php') !== false) {
                        unset($array[$index]);
                    }
                }
