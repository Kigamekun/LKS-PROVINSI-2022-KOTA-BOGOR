...
class HMWP_Controllers_SecurityCheck extends HMWP_Classes_FrontController
{
    ...
    public function getTasks()
    {
        return array(
            ...
            'checkAdminPath' => array(
                ...
                'valid' => false,
                'warning' => false,
                'message' => sprintf(__("Having the admin URL visible in the source code it's awful because hackers will immediately know your secret admin path and start a Brute Force attack. The custom admin path should not appear in the ajax URL. <br /><br />Find solutions for %s how to hide the path from source code %s.", 'hide-my-wp'), '<a href="'.HMWP_Classes_Tools::getOption('hmwp_plugin_website').'/how-to-hide-admin-and-wp-login-php-from-source-code/" target="_blank">', '</a>'),
                'solution' => sprintf(esc_html__("Switch on %s %s > Change Paths > Hide admin from ajax URL%s. Hide any reference to admin path from the installed plugins.", 'hide-my-wp'), '<a href="'.HMWP_Classes_Tools::getSettingsUrl('hmwp_permalinks#tab=ajax').'" >', HMWP_Classes_Tools::getOption('hmwp_plugin_menu'), '</a>', '<strong>', '</strong>'),
            ),
            'checkLoginPath' => array(
                ...
                'valid' => false,
                'warning' => false,
                'message' => sprintf(__("Having the login URL visible in the source code is awful because hackers will immediately know your secret login path and start a Brute Force attack. <br /><br />The custom login path should be kept secret, and you should have Brute Force Protection activated for it. <br ><br />Find solutions for %s hiding the login path from source code here %s.", 'hide-my-wp'), '<a href="'.HMWP_Classes_Tools::getOption('hmwp_plugin_website').'/how-to-hide-admin-and-wp-login-php-from-source-code/" target="_blank">', '</a>'),
            ...
            'checkInstall' => array(
                ...
                'valid' => false,
                'warning' => false,
                'message' => __( "WordPress is well-known for its ease of installation. <br/>It's important to hide the admin/install.php and admin/upgrade.php files because there have already been a couple of security issues regarding these files.", 'hide-my-wp' ),
                'solution' => sprintf( esc_html__( "Rename admin/install.php & admin/upgrade.php files or switch on %s %s > Change Paths > Hide WordPress Common Paths%s", 'hide-my-wp' ), '<a href="'.HMWP_Classes_Tools::getSettingsUrl('hmwp_permalinks#tab=core').'" >',  HMWP_Classes_Tools::getOption('hmwp_plugin_menu') , '</a>'),
                'javascript' => "pro",
            ),
    ...
    {
        $url = site_url() . '/admin/install.php?rnd=' . rand();
        $response = wp_remote_head($url,  array('redirection' => 0, 'timeout' => 5, 'cookies' => false));
    ...
    /**
     * Check if admin is accessible for visitors
     *
     * @return array
     */
