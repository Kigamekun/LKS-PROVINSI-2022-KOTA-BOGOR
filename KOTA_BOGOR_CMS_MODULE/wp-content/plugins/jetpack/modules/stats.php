 ...
 * See admin/admin.php where it checks $wp_db_version.
 *
 * @access public
...
function stats_admin_menu() {
	...
	if ( 'index.php' === $pagenow && isset( $_GET['page'] ) && 'stats' === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$redirect_url = str_replace( array( '/admin/index.php?', '/admin/?' ), '/admin/admin.php?', isset( $_SERVER['REQUEST_URI'] ) ? filter_var( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : null );
		$relative_pos = strpos( $redirect_url, '/admin/' );
...
function stats_reports_page( $main_chart_only = false ) {
	...
	if ( isset( $_GET['chart'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		$url = 'https://' . STATS_DASHBOARD_SERVER . '/admin/index.php';
	}
...
#wpadminbar .quicklinks li#admin-bar-stats {
	height: 32px;
}
#wpadminbar .quicklinks li#admin-bar-stats a {
	height: 32px;
	padding: 0;
}
#wpadminbar .quicklinks li#admin-bar-stats a div {
	height: 32px;
	width: 95px;
	overflow: hidden;
	margin: 0 10px;
}
#wpadminbar .quicklinks li#admin-bar-stats a:hover div {
	width: auto;
	margin: 0 8px 0 10px;
}
#wpadminbar .quicklinks li#admin-bar-stats a img {
	height: 24px;
	margin: 4px 0;
	max-width: none;
	border: none;
...
/**
 * Load CSS needed for Stats column width in admin area.
 *
 * @since 4.7.0
 */
