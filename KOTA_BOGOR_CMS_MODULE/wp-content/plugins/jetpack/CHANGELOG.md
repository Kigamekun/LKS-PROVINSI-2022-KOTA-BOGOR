...
- Masterbar: make the Desktop Switcher look the same between Calypso and admin.
...
- Fixed nav-unification on lower resolutions for admin.
...
- Update "Appearance > Widgets" and "Appearance > Menus" links back to classic admin destination for Atomic and Simple sites.
- Updated package dependencies
...
- Search: visual, functional, and accessibility improvements to new admin section and customization interface.
...
- Facebook Page Widget: fix a bug that prevented widget from being added in the admin widget editor.
...
- Social Icons Widget: fix issue in admin widget editor where icons don't save on refresh.
...
- Fix submenu positioning issue for Nav-Unification in admin.
...
- WoA: fix My Sites navigation in admin for Atomic sites with SSO disabled (and nav-unification disabled).
...
- Fixed regression introduced in posts page icon notification admin edit.php page.
...
- Disabled editing and deleting pages in admin Pages list screen for the page that is configured as a Posts page.
...
- Ensure that the WPCOM toolbar remains enabled with AMP in admin
...
- Fixed the Upgrades, Jetpack and Settings menu item slugs in admin
...
### Bug fixes
- Instant Search: prevent IE11 from spawning overlay on load.
- WordPress.com Toolbar and customizations: fix multiple issues causing navigation panel discrepancies between the admin dashboard and the WordPress.com dashboard.
...
- Subscriptions: add a admin setting to alert the admin when someone follows the blog.
...
- WordPress.com Toolbar: Add menu icon for smaller screen widths to restore admin navigation menu.
...
- Sharing: Sharing section in admin will now redirect to Calypso instead.
...
- General: We solved an issue that arised when using the Front End Editor feature plugin. A fatal error wass thrown due to us assuming the `enter_title_here` filter would only run within admin.
...
- Shortcodes: Fixed the Facebook shortcode in admin.
...
- Publicize: Made styling of Publicize more consistent with admin.
...
- Bug Fix: Notifications: Notifications didn't load on admin/network pages
...
- Bug Fix: Javascript errors on admin due to stats display code
...
- Enhancement: Likes: Added the number of likes to the admin/edit.php
  screens.
